...
const D10 = forwardRef(({ onRest, isRolling, serverRoll, ...props }, ref) => {
  ...
  const marbleTexture = useMemo(() => {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      const r = Math.random();
      if (r < 0.25) {
        grad.addColorStop(0, 'rgba(204, 251, 241, 0.25)'); // Light seafoam/mint
      } else if (r < 0.5) {
        grad.addColorStop(0, 'rgba(4, 47, 46, 0.65)');      // Deep dark teal vein
      } else if (r < 0.75) {
        grad.addColorStop(0, 'rgba(13, 148, 160, 0.45)');   // Vibrant oceanic teal
      } else {
        grad.addColorStop(0, 'rgba(15, 118, 110, 0.55)');    // Mid teal
      }
